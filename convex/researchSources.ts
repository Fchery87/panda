...
const RecencyFilter = v.union(v.literal('day'), v.literal('week'), v.literal('month'), v.literal('year'))
...
function configuredProvider(requested?: 'auto' | 'exa' | 'perplexity' | 'gemini'):
  | 'exa'
  | 'perplexity'
  | 'gemini'
  | null {
...
async function searchExa(query: string, numResults: number, domainFilter?: string[]): Promise<SearchPayload> {
  ...
  const excludeDomains = domainFilter?.filter((domain) => domain.startsWith('-')).map((domain) => domain.slice(1))
  const res = await fetch('https://api.exa.ai/search', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-api-key': key },
    body: JSON.stringify({ query, numResults, ...(includeDomains?.length ? { includeDomains } : {}), ...(excludeDomains?.length ? { excludeDomains } : {}) }),
  })
  ...
  const results = (json.results ?? [])
    ...
    .map((item) => ({ title: item.title || item.url!, url: item.url!, snippet: item.snippet ?? item.text?.slice(0, 300) }))
...
async function searchGemini(query: string, numResults: number): Promise<SearchPayload> {
  ...
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`, {
    ...
    body: JSON.stringify({
      contents: [{ parts: [{ text: `Search/summarize the web for this query and include source URLs when possible. Query: ${query}` }] }],
      tools: [{ google_search: {} }],
    }),
  })
  if (!res.ok) throw new Error(`Gemini search failed with HTTP ${res.status}`)
  const json = (await res.json()) as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> }; groundingMetadata?: { groundingChunks?: Array<{ web?: { uri?: string; title?: string } }> } }> }
...
async function searchWithProvider(args: {
  ...
  if (args.provider === 'exa') return await searchExa(args.query, args.numResults, args.domainFilter)
...
async function extractPdfViaJina(url: string): Promise<string | null> {
  try {
    ...
    const markdown = marker >= 0 ? text.slice(marker + 'Markdown Content:'.length).trim() : text.trim()
...
export const listByProject = query({
  args: { projectId: v.id('projects'), chatId: v.optional(v.id('chats')), limit: v.optional(v.number()) },
  ...
  handler: async (ctx, args): Promise<{
    ...
    if (!provider) {
      return {
        ...
        message: 'Configure EXA_API_KEY, PERPLEXITY_API_KEY, or GEMINI_API_KEY in Convex environment variables to enable research_web_search.',
    ...
    for (const query of queries) {
      const payload = await searchWithProvider({ provider, query, numResults, domainFilter: args.domainFilter })
      const citations = payload.results.map((result) => ({ title: result.title, url: result.url, snippet: result.snippet }))
      const markdown = [`# Web search: ${query}`, '', payload.answer, '', '## Sources', ...payload.results.map((result, index) => `${index + 1}. [${result.title}](${result.url})${result.snippet ? ` — ${result.snippet}` : ''}`)].join('\n')
      ...
      if (args.includeContent === true) {
        for (const result of payload.results.slice(0, Math.min(3, numResults))) {
          try {
            ...
            const res = await fetch(parsed.toString(), { headers: { accept: 'text/html, text/plain, */*;q=0.5' } })
            if (!res.ok) continue
            const contentType = res.headers.get('content-type') ?? ''
            const raw = await res.text()
            const markdown = contentType.includes('html') ? stripHtml(raw) : normalizeText(raw)
            const pageTitle = contentType.includes('html') ? titleFromHtml(raw, result.title) : result.title
      ...
      queryResults.push({ query, answer: payload.answer, provider: payload.provider, sourceId, sources: citations })
    }
...
export const fetchUrl = action({
  ...
  handler: async (ctx, args): Promise<{ status: 'ok'; sourceId: string; kind: string; title: string; url: string; summary: string; contentPreview: string; truncated: boolean } | { status: 'error'; error: string }> => {
    try {
      const parsed = new URL(args.url)
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') throw new Error('Only http(s) URLs are supported')
      const controller = new AbortController()
      const timer = setTimeout(() => controller.abort(), Math.min(Math.max(args.timeoutMs ?? 30000, 1000), 60000))
      try {
        const res = await fetch(parsed.toString(), { signal: controller.signal, headers: { accept: 'text/html, text/plain, application/pdf;q=0.8, */*;q=0.5' } })
        ...
        const markdown = isPdf
          ...
          : contentType.includes('html')
            ? stripHtml(raw)
            : normalizeText(raw)
        const title = isPdf ? parsed.pathname.split('/').pop() || parsed.toString() : contentType.includes('html') ? titleFromHtml(raw, parsed.hostname) : parsed.pathname.split('/').pop() || parsed.hostname
        ...
        return { status: 'ok', sourceId, kind, title, url: parsed.toString(), summary: summarize(markdown), contentPreview, truncated: markdown.length > contentPreview.length }
      } finally {
        clearTimeout(timer)
      }
    } catch (error) {
      return { status: 'error', error: error instanceof Error ? error.message : 'Failed to fetch URL' }
    }
  },
})
  ...
  handler: async (ctx, args): Promise<{ status: 'ok'; sourceId: string; kind: string; title: string; url: string; summary: string; contentPreview: string; truncated: boolean } | { status: 'error'; error: string }> => {
    try {
      ...
      if (info.mode === 'blob' && info.path) {
        ...
        const repo = await githubApi(`/repos/${info.owner}/${info.repo}`) as { default_branch?: string; description?: string; full_name?: string }
        ...
        let readme = ''
        try { const res = await fetch(`https://raw.githubusercontent.com/${info.owner}/${info.repo}/${ref}/README.md`); if (res.ok) readme = await res.text() } catch { readme = '' }
        ...
        markdown = [`# ${title}`, repo.description ?? '', info.path ? `Directory focus: ${info.path}` : '', '## README', readme || '(README.md not found)', '## Tree preview', paths.join('\n')].join('\n\n')
      }
      const sourceId = await ctx.runMutation(api.researchSources.create, { projectId: args.projectId, chatId: args.chatId, kind, url: args.url, title, provider: 'github', contentHash: await hashText(markdown), extractedMarkdown: markdown, summary: summarize(markdown) })
      const contentPreview = summarize(markdown, 6000)
      return { status: 'ok', sourceId, kind, title, url: args.url, summary: summarize(markdown), contentPreview, truncated: markdown.length > contentPreview.length }
    } catch (error) {
      return { status: 'error', error: error instanceof Error ? error.message : 'Failed to fetch GitHub source' }
    }
  },
})
