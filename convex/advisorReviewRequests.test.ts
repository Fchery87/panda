...
describe('advisor review requests convex contract', () => {
  test('defines request table and create mutation', () => {
    ...
    const source = fs.readFileSync(path.resolve(import.meta.dir, 'advisorReviewRequests.ts'), 'utf8')

    expect(schema).toContain('advisorReviewRequests: defineTable')
    expect(schema).toContain("status: v.union(v.literal('pending'), v.literal('completed'), v.literal('cancelled'))")
