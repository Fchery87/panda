...
const TEXT_EXTENSIONS = new Set([
  ...
  // Other
  '.dockerfile', '.gitignore', '.env', '.env.example',
  '.lock', '.gitattributes', '.editorconfig',
]);
...
export const importRepo = action({
  ...
  handler: async (ctx, { repoUrl, projectId, branch }): Promise<ImportProgress> => {
    ...
    try {
      ...
      if (!parsed) {
        throw new Error('Invalid GitHub URL. Expected format: github.com/owner/repo or https://github.com/owner/repo');
      }
