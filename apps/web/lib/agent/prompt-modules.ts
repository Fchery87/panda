...
/**
 * Static and context-dependent Prompt System modules.
 *
 * `prompt-library.ts` owns public message assembly. This file owns reusable
 * prompt sections so shared contracts do not drift across Chat Modes.
 */

export const BROWSER_ENVIRONMENT_CONTEXT = `## Environment Context
...
export const ARCHITECT_SYSTEM_PROMPT = `You are Panda.ai, a senior software architect.
...
export const CODE_SYSTEM_PROMPT = `You are Panda.ai, a senior software engineer.
...
export const BUILD_SYSTEM_PROMPT = `You are Panda.ai, a senior software engineer executing a full build.
...
export const ARCHITECT_BRAINSTORM_PROTOCOL = `
...
export function buildPlanningSessionContext(context: PromptContext): string {
...
export function buildApprovedPlanExecutionContext(context: PromptContext): string {
  if (
...
export function buildActiveSpecContext(context: PromptContext): string {
  ...
  return [
    ...
    'Spec is the stronger execution contract when it conflicts with an approved Plan.',
