...
import type { CompletionOptions, LLMProvider, ProviderConfig, StreamChunk, ToolCall } from "../llm/types";
    ...
    const provider: LLMProvider = {
      ...
      async *completionStream(_options: CompletionOptions): AsyncGenerator<StreamChunk> {
        ...
        if (callCount === 2) {
          ...
          yield { type: "tool_call", toolCall: makeToolCall("write_files", { files: [{ path: "x.ts", content: "export const x = 1\n" }] }) };
          yield makeFinish();
          return;
        }
    ...
    const provider: LLMProvider = {
      ...
      async *completionStream(_options: CompletionOptions): AsyncGenerator<StreamChunk> {
        ...
        if (callCount === 1) {
          ...
          yield { type: "text", content: "### Proposed Plan\n1) Do thing\n### Next Step\nI will begin by writing files.\n" };
          yield makeFinish();
          return;
        }
