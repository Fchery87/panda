...
const DEFAULT_RULES: Record<ChatMode, PermissionRule[]> = {
  ...
  plan: [
    ...
    {
      capability: 'plan_exit',
      decision: 'ask',
      source: 'mode',
      reason: 'plan exit requires user confirmation',
    },
  ],
  code: [
    { capability: 'read', decision: 'allow', source: 'mode' },
    ...
    { capability: 'memory', decision: 'allow', source: 'mode' },
    {
      ...
      decision: 'ask',
      source: 'mode',
      reason: 'file writes require approval in code mode',
    },
    {
      capability: 'exec',
      decision: 'ask',
      source: 'mode',
      reason: 'commands require approval in code mode',
    },
  ],
  build: [
    ...
    // Dangerous commands still require approval even in build mode
    {
      ...
      source: 'mode',
      pattern: 'rm *',
      reason: 'rm commands require approval',
    },
    {
      capability: 'exec',
      decision: 'ask',
      source: 'mode',
      pattern: 'rm -rf *',
      reason: 'rm -rf commands require approval',
    },
    {
      capability: 'exec',
      decision: 'ask',
