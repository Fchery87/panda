  ...
  try {
    ...
    if (isPdf) {
      ...
      return {
        kind: 'pdf',
        url: parsed.toString(),
        title: parsed.pathname.split('/').pop() || parsed.toString(),
        markdown,
    ...
    return {
      ...
      title: contentType.includes('html')
        ? titleFromHtml(text, parsed.hostname)
...
export async function extractGitHubContent(url: string): Promise<{
  ...
  const markdown = [
    `# ${repo.full_name ?? `${info.owner}/${info.repo}`}`,
    repo.description ?? '',
    pathFilter,
    '## README',
...
export function toResearchToolResult(args: {
  sourceId: string
  kind: ResearchSourceKind
  title?: string
  url: string
