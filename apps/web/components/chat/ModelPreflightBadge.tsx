...
export function ModelPreflightBadge({ preflight }: ModelPreflightBadgeProps) {
  return (
    <div
      className={cn(
        'flex min-w-0 flex-wrap items-center gap-1.5 border-t border-border px-2 py-1 font-mono text-[10px]',
    ...
    >
      ...
      <span className="hidden xl:inline">{preflight.context}</span>
      <span className="hidden 2xl:inline">{preflight.cost}</span>
      <span className="hidden 2xl:inline">{preflight.reasoning}</span>
