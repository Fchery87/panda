...
describe('project history flow', () => {
  test('chat history action opens the persisted run history rail', () => {
    ...
    const runtimeValuePath = path.resolve(
      import.meta.dir,
      '../../hooks/useWorkspaceRuntimeValue.ts'
    )
    const runtimeValueSource = fs.readFileSync(runtimeValuePath, 'utf8')

    expect(panelSource).toContain('onClick={onOpenHistory}')
    expect(panelSource).toContain('Run History ({runHistoryCount})')

    expect(runtimeValueSource).toContain("onOpenHistory: () => openRightPanelTab('proof')")
  })
})
